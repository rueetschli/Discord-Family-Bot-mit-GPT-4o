async def handle_audio_message(message: discord.Message, attachment: discord.Attachment):
    ...
    try:
        ...
        if hasattr(ans, "audio") and ans.audio is not None:
            ...
            user_transcript = ans.content

        if user_transcript:
            # Speichere die Benutzeranfrage (transkribierter Text) in der DB
        ...
        if audio_info and audio_info.data:
            ...
            logger.info(f"[Text-Antwort gesendet] user_id={user_id}, message_id={message.id}")
...
1. **Sicheres Extrahieren des transkribierten Textes:**
   - Überprüft, ob `ans.audio` existiert und nicht `None` ist, bevor auf `transcript` zugegriffen wird.
   - Falls `ans.audio` nicht vorhanden ist, wird versucht, den transkribierten Text aus `ans.content` zu extrahieren.
   - Falls keine Transkription erkannt wird, wird eine Warnung geloggt.

2. **Speichern der Benutzeranfrage:**
   - Wenn eine Transkription vorhanden ist, wird diese als Benutzeranfrage in der Datenbank gespeichert.
   - Dies stellt sicher, dass auch Voice-Chat-Nachrichten in der Datenbank erfasst werden und somit der Kontext korrekt bleibt.

3. **Verbesserte Fehlerbehandlung:**
   - Wenn keine Transkription erkannt wird, wird dies geloggt, und der Bot sendet eine entsprechende Meldung zurück.

### **Schritt 4: Aktualisierte `main.py` mit allen Änderungen**

Hier ist die vollständige, aktualisierte `main.py`, die alle oben genannten Änderungen beinhaltet:
